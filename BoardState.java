...
public class BoardState {
    ...
    public boolean checkLegalMove(int x, int y) {
        return checkOnBoard(x, y) && (getContents(x, y) == 0)
                && ((checkLeft(x, y) != -1) || (checkRight(x, y) != -1) || (checkUp(x, y) != -1)
                        || (checkDown(x, y) != -1) || (checkUpLeft(x, y) != -1) || (checkUpRight(x, y) != -1)
    ...
    // j (i < j < endOfBoard) and board[i,y] is colour, and return -1 if none in
    // range
